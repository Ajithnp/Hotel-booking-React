...
const RoomDetails = () => {
  ...
  return room && (
    <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32'>
        ...
        <div className='grid grid-cols-2 gap-4 lg:w-1/2 w-full'>
            {room?.images.length > 1 && room.images.map((image,index)=> (
                <img onClick={()=> setMainImage(image)}
                 ...
                 className={`w-full rounded-xl shadowmd object-cover
        ...
        <div className='flex flex-col'>
            ...
            <div className='flex flex-wrap items-center mt-3 mb-6 gap-4'>
                {room.amenities.map((item,index)=> (
                    <div key={index } className='flex items-center gap-2 px-3 my-2 rounded-lg bg-gray-100'>
      ...
      {/* Ckekin Checkout form */}
      <form className='flex flex-col md:flex-row items-start md:items-center justify-between bg-white shadow-[0px_0px_20px_rgba(0,0,0,0.15)] p-6 rounded-xl mx-auto mt-16 max-w-6xl'>
        <div className='flex flex-col flex-wrap md:flex-row items-start md:items-center gap-4 md:gap-10 text-gray-500'>

            <div className='flex flex-col'>
                <label htmlFor="ckeckInDate" className='font-medium'>CheckIn</label>
                <input type="date" id='ckeckInDate' placeholder='Check-In'
            ...
            <div className='flex flex-col'>
                <label htmlFor="ckeckOutDate" className='font-medium'>CheckOut</label>
                <input type="date" id='ckeckOutDate' placeholder='Check-Out'
                className='w-full rounded border border-gray-300 px-3 py-2 mt-1.5 outline-none' required/>
            </div>
        ...
        <button type='subbmit' className='bg-primary hover:bg-primary-dull active:scale-95
      ...
      <div className='max-w-3xl border-y border-gray-300 my-15 py-10 text-gray-500'>
        <p>Guest will be allocated on the ground floor according to availabiloty.
            You get a comfortable Two Bedroom apartment fjhgf fjhsxhyygxc  cjhxgfcxvfc 
            fhbfvdgfdf fjhsgfjhsdf fjhsgfjsdf fhsfjsdf fhbfnsf  fhsdhf ksf sdgf  sf shf
        </p>
      </div>
